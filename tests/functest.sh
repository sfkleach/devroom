...
# Assert that running cmd produces output matching grep pattern.
assert_output() {
...
assert_output   "init reports that config already exists" "already exists" \
    "$DEVROOM" init --rootdir "$d"
rm -rf "$d"
